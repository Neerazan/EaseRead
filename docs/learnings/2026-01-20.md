...
### High-Level Workflow
Logging out involves two main responsibilities:
1. **Server-side cleanup**: Invalidating the refresh token to prevent it from being used again.
2. **Client-side cleanup**: Clearing the cookies in the user's browser.

### Invalidation Strategy
Since we store Refresh Token IDs in Redis, we simply delete the key associated with the user:
- `this.redisClient.del(this.getKey(userId))`
Once deleted, even if the user has a valid-looking Refresh Token cookie, the server will reject it because the ID is no longer in the whitelist.

### Cookie Management
Using NestJS `@Res({ passthrough: true })`, we can manually clear cookies by calling:
- `response.clearCookie('access_token')`
- `response.clearCookie('refresh_token')`

### Security Note
Access Tokens (JWTs) cannot be easily invalidated before they expire because they are stateless. This is why it's critical to keep Access Token TTLs short (e.g., 15 minutes) and rely on the stateful Refresh Token for session management.
